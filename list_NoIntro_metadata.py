...
for rom_key in sorted(main_rom_set):
    ...
    hl_nplayers_srt = hyperlist_dic[rom_key]['player'] if rom_key in hyperlist_dic else ''
    hl_rating_srt   = hyperlist_dic[rom_key]['rating'] if rom_key in hyperlist_dic else ''
    hl_plot_srt     = hyperlist_dic[rom_key]['story'] if rom_key in hyperlist_dic else ''
    ...
    m_studio_srt   = text_limit_string(metadata['studio'], PUBLISHER_LENGTH)
    ...
    print(line_4.format(metadata_status.ljust(DB_LENGTH),
                        ...
                        m_studio_srt.ljust(PUBLISHER_LENGTH), m_nplayers_srt.ljust(NPLAYERS_LENGTH),
                        m_rating_srt.ljust(ESRB_LENGTH), m_plot_srt.ljust(PLOT_LENGTH) ))

    metadata['source'] = metadata_status
...
for rom_key in sorted(metadata_dic):
    ...
    str_list.append(XML_text('description',  text_str_2_Uni(metadata_dic[rom_key]['description'])))
    str_list.append(XML_text('year',         text_str_2_Uni(metadata_dic[rom_key]['year'])))
    str_list.append(XML_text('rating',       text_str_2_Uni(metadata_dic[rom_key]['rating'])))
    str_list.append(XML_text('manufacturer', text_str_2_Uni(metadata_dic[rom_key]['manufacturer'])))
    str_list.append(XML_text('dev',          text_str_2_Uni(metadata_dic[rom_key]['dev'])))
    str_list.append(XML_text('genre',        text_str_2_Uni(metadata_dic[rom_key]['genre'])))
    str_list.append(XML_text('score',        text_str_2_Uni(metadata_dic[rom_key]['score'])))
    str_list.append(XML_text('player',       text_str_2_Uni(metadata_dic[rom_key]['player'])))
    str_list.append(XML_text('story',        text_str_2_Uni(metadata_dic[rom_key]['story'])))
    str_list.append(XML_text('cloneof',      text_str_2_Uni(metadata_dic[rom_key]['cloneof'])))
    str_list.append(XML_text('source',       text_str_2_Uni(metadata_dic[rom_key]['source'])))
